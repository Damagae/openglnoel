...
void Application::initScene(const glmlv::fs::path & objPath)
{
...

    {
        ...
        std::cout << "# of animations   : " << m_Model.meshes.size() << std::endl;
        ...
        // -- model textures
        std::vector<GLint> textureIds; // = gMeshState[mesh.name].diffuseTex[primId]
        auto scene = m_Model.scenes[m_Model.defaultScene > -1 ? m_Model.defaultScene : 0];

        for (size_t i = 0; i < m_Model.meshes.size(); i++) {

          const tinygltf::Mesh &mesh = m_Model.meshes[i];
          m_GMeshState[mesh.name].diffuseTex.resize(mesh.primitives.size());

          for (size_t primId = 0; primId < mesh.primitives.size(); primId++) {

            const tinygltf::Primitive &primitive = mesh.primitives[primId];

            m_GMeshState[mesh.name].diffuseTex[primId] = 0;
            ...
            // std::cout << "size = " << mat.values.size() << std::endl;
            // auto value = mat.values["diffuse"].string_value;
            // std::cout << "value = " << value.length() << std::endl;
            ...
            if (mat.values["diffuse"].string_value.length() != 0) {

              std::string diffuseTexName = mat.values["diffuse"].string_value;

              if (/*findIteratorInVector(m_Model.textures, diffuseTexName) != m_Model.textures.end()*/true) {

                tinygltf::Texture tex = findElementInVector(m_Model.textures, diffuseTexName); // &tex

                if (/*findIteratorInVector(m_Model.images, tex.source) != m_Model.images.end()*/true) {
                  ...
                  // glTexImage2D(tex.target, 0, tex.internalFormat, image.width, image.height, 0, format, tex.type, &image.image.at(0));
                  ...
                  m_GMeshState[mesh.name].diffuseTex[primId] = texId;
                }
              }
            }
          }
